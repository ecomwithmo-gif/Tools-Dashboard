...
import { Download, Play, AlertCircle, CheckCircle, XCircle, Search, Mail, StopCircle, RefreshCw, BarChart } from 'lucide-react';
...
function App() {
  ...
  const handleExport = () => {
    const csvContent = [
      ...
      ...results.flatMap(r => 
        r.emails.length > 0 
          ? r.emails.map(e => [r.url, e.email, e.valid_mx ? 'Yes' : 'No', r.scraped_pages || 1]) 
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-50 text-slate-900 font-sans overflow-hidden selection:bg-indigo-100 selection:text-indigo-700">
